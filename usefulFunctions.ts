...
// Мотивационные подписи кнопки "далее"/"дальше" после ВЕРНОГО ответа —
// общие и для тренажёра (components/trainer-question.tsx), и для
// обычного задачника (app/lesson/footer.tsx), чтобы не разводить два
// разных списка ради одной и той же идеи. По просьбе пользователя —
// вместо всегда одинакового "далее" с некоторой вероятностью показываем
// что-то, что поднимает мотивацию/самооценку.
export const MOTIVATIONAL_NEXT_PHRASES = [
    'я смогу!',
    'я красавчик!',
    'я мегамозг!',
    'я гений!',
...
// defaultLabel — обычное "далее"/"дальше" (разное написание в тренажёре
// и задачнике), chance — как часто вместо него показывать мотивационную
// фразу (по умолчанию треть кликов — часто достаточно, чтобы быть
// заметным, но не настолько, чтобы обычное "далее" совсем потерялось).
export const pickNextButtonLabel = (defaultLabel: string, chance: number = 0.35): string => {
    if (Math.random() < chance) {
        return MOTIVATIONAL_NEXT_PHRASES[Math.floor(Math.random() * MOTIVATIONAL_NEXT_PHRASES.length)];
    }
    return defaultLabel;
};