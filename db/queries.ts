...
export const getTLesson = cache(async (t_lessonId: number) => {
  ...
  const data = await db.query.t_lessons.findFirst({
    ...
    with: {
      ...
      // Соседние этапы темы — id/order/title нужны для кнопки "Следующий
      // урок" на финальном экране (TQUIZ.tsx). Полные t_challenges (+
      // options) КАЖДОГО этапа темы — для построения пула обманок в
      // page.tsx на уровне ВСЕЙ темы, а не только текущего урока (см.
      // topicChallenges там же): у маленьких словарных уроков (5-8 задач)
      // "своего рода" (та же единица измерения/то же название величины)
      // кандидата иногда попросту нет ВНУТРИ одного урока — например у
      // урока с одной лишь парой "F_тяж"/"F_упр" единственный сосед-
      // единица для вопроса "В чём измеряется F_упр?" (id F_тяж, тоже
      // "Н") исключается как дубль-по-значению, и без соседних уроков
      // пул обманок пуст, откатываясь на дистракторы чужого рода (имена
      // вместо единиц измерения) — баг, пойманный пользователем живьём.
      t_unit: {
        with: {
          t_lessons: {
            orderBy: (l, { asc }) => [asc(l.order)],
