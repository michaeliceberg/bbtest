...
export type allTypesCT = "M_ASC" | "SELECT" | "ASSIST" | "CONNECT" | "SLIDER" | "CONSTRUCT" | "WORKBOOK" | "R ASSIST" | "R CONNECT" | "R SLIDER" | "GEOSIN" | "RUSSIANDICTANT" | "SWIPE" | "KEYBOARD" | "INSERT" | "SCROLL" | "HOT" | "CHECK" | "PICMATCH" | "MULTISTEP" | "SPEED" | "FRACTRICK" | "DIAGRAM" | "TRIGTABLE" | "UNITCIRCLE" | "VIETA" | "SINWALK" | "LOGWALK" | "LOGDEFWALK" | "LOGSUBWALK" | "LOGPOWWALK" | "LOGSWAPWALK" | "LOGDIVWALK" | "LOGCOMBOWALK" | "LOGFLIPWALK" | "FARADAYWALK" | "DIRWALK" | "SINCOSDEFWALK" | "LEGFINDWALK" | "TRIGSCWALK" | "TRIGTGWALK" | "TRIGCIRCWALK";
...
export const t_challengesEnum = pgEnum("type", [
	"M_ASC", "SELECT", "ASSIST", "CONNECT", "SLIDER", "CONSTRUCT", "WORKBOOK",
	"R ASSIST", "R CONNECT", "R SLIDER", "GEOSIN", "RUSSIANDICTANT", "SWIPE", "KEYBOARD",
	"INSERT", "SCROLL", "CHECK", "PICMATCH", "MULTISTEP", "SPEED", "FRACTRICK", "DIAGRAM", "TRIGTABLE", "UNITCIRCLE", "VIETA", "SINWALK", "LOGWALK", "LOGDEFWALK", "LOGSUBWALK", "LOGPOWWALK", "LOGSWAPWALK", "LOGDIVWALK", "LOGCOMBOWALK", "LOGFLIPWALK", "FARADAYWALK", "DIRWALK", "SINCOSDEFWALK", "LEGFINDWALK", "TRIGSCWALK", "TRIGTGWALK", "TRIGCIRCWALK"
]);
...
export const t_units = pgTable('t_units', {
	...
	// {"Таблица 30,45,60", "Геометрия: sin,cos,tg"} → "Как записать ответ").
...
export const t_challenges = pgTable('t_challenges', {
	...
	// Только для type='MULTISTEP' — JSON-массив шагов (см. MultistepStep в
	// app/t-lesson/[t_lessonId]/page.tsx): каждый шаг — своя подсказка,
	// формула и правильный ответ, вводимый пользователем цифрами.
	// Nullable — у всех остальных типов не используется.
	multistepData: text('multistep_data'),
	// Только для type='FRACTRICK' — JSON с данными "трюка умножения/деления
	// на унитарную дробь" (см. FracTrickData в app/t-lesson/[t_lessonId]/page.tsx):
	...
	// Только для type='UNITCIRCLE' — JSON с данными тригонометрического
	// круга (см. UnitCircleData в app/t-lesson/[t_lessonId]/page.tsx):
	// режим (locate/select), список точек-магнитов (LaTeX-подпись угла +
