// scripts/seedEGEPhysics-unit1-lesson2.ts
//
// Курс "ЕГЭ Физика" — Unit 1 "Кинематика", урок 2
// (22 задачи с https://phys-ege.sdamgia.ru/test?theme=202:
// относительность движения, графики координаты).
...
const challenges: ChallengeSeed[] = JSON.parse(
    readFileSync('/tmp/phys204_work/challenges202.json', 'utf-8')
);
...
const main = async () => {
    try {
        console.log('Seeding: ЕГЭ Физика → Unit 1 → Кинематика → Урок 2');

        const unit = await db.query.units.findFirst({ where: eq(schema.units.title, 'Кинематика') });
        if (!unit) throw new Error('Unit "Кинематика" не найден — сначала запустите seedEGEPhysics-unit1-lesson1.ts');

        const [lesson] = await db.insert(schema.lessons).values({
            title: 'Относительность движения и графики координаты',
            unitId: unit.id,
            order: 2,
        }).returning();
