...
// - "Как записать ответ" (изначально засеяна под именем "Как писать
//   ответ", переименован сам t_unit + все строки справочника позже,
//   2026-09-17) — механически вытянуты ВСЕ 48 задач юнита (каждое
...
//   (не придуманы с нуля).
//
// ВАЖНО: содержимое "Геометрия: sin, cos, tg" ниже в этом файле УЖЕ НЕ
// АКТУАЛЬНО — 2026-09-17 6 исходных строк заменены на 10 новых (диаграмма
// прямоугольного треугольника a/b/c/α + 3 определения через отношения
// сторон + те же 6 формул "найти сторону"), точечной разовой миграцией
// (не сохранена — см. CLAUDE.md). Блок ниже оставлен только как
// исторический снимок первой версии, этот файл повторно не запускается
// (guard "уже есть строки" в main()).
//
