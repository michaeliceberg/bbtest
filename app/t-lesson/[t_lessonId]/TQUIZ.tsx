  ...
  const handleAnswer = useCallback(async (answer: string) => {
    ...
    try {
      ...
      if (answerIsRight) {
        ...
        // LOGSWAPWALK: это самодостаточные пошаговые разборы, которые уже
        // сами отслеживают и повторяют свои внутренние ошибки (см.
        // type-sinwalk.tsx/type-logwalk.tsx/type-logdefwalk.tsx/
        // type-logsubwalk.tsx/type-logpowwalk.tsx/type-logswapwalk.tsx,
        // hadMistake/доп. попытки) — onAnswer('wrong') здесь означает "по
        // пути были помарки", а не "непонятно, надо переспросить", поэтому
        // ставить его в очередь "работы над ошибками" (полный повтор всего
        // разбора заново) избыточно и сбивает с толку — лессон просто
        // должен закончиться по клику "Готово".
        if (questions[currentQuestionIndex].questionType !== 'SINWALK' && questions[currentQuestionIndex].questionType !== 'LOGWALK' && questions[currentQuestionIndex].questionType !== 'LOGDEFWALK' && questions[currentQuestionIndex].questionType !== 'LOGSUBWALK' && questions[currentQuestionIndex].questionType !== 'LOGPOWWALK' && questions[currentQuestionIndex].questionType !== 'LOGSWAPWALK') {
