...
// непрерывную серию.
// 7 подряд убран по просьбе пользователя (2026-09-26) — поздравление только на 3.
const STREAK_MILESTONES = [3] as const
...
export default function TQuiz({
  ...
  const handleAnswer = useCallback(async (answer: string) => {
    ...
    try {
      ...
      // любой из нескольких верных синонимов (например, "Дж" — и работа,
      // и энергия).
      ...
      if (answerIsRight) {
        ...
        // ИСКЛЮЧЕНИЕ — SINWALK/LOGWALK/LOGDEFWALK/LOGSUBWALK/LOGPOWWALK/
        // LOGSWAPWALK/LOGDIVWALK/LOGCOMBOWALK/LOGFLIPWALK: это
        // самодостаточные пошаговые разборы, которые уже сами отслеживают
        // и повторяют свои внутренние ошибки (см. type-sinwalk.tsx/
        // type-logwalk.tsx/type-logdefwalk.tsx/type-logsubwalk.tsx/
        // type-logpowwalk.tsx/type-logswapwalk.tsx/type-logdivwalk.tsx/
        // type-logcombowalk.tsx/type-logflipwalk.tsx, hadMistake/доп.
        // попытки) — onAnswer('wrong')
        ...
        // "Готово".
        if (questions[currentQuestionIndex].questionType !== 'SINWALK' && questions[currentQuestionIndex].questionType !== 'LOGWALK' && questions[currentQuestionIndex].questionType !== 'LOGDEFWALK' && questions[currentQuestionIndex].questionType !== 'LOGSUBWALK' && questions[currentQuestionIndex].questionType !== 'LOGPOWWALK' && questions[currentQuestionIndex].questionType !== 'LOGSWAPWALK' && questions[currentQuestionIndex].questionType !== 'LOGDIVWALK' && questions[currentQuestionIndex].questionType !== 'LOGCOMBOWALK' && questions[currentQuestionIndex].questionType !== 'LOGFLIPWALK' && questions[currentQuestionIndex].questionType !== 'FARADAYWALK' && questions[currentQuestionIndex].questionType !== 'DIRWALK' && questions[currentQuestionIndex].questionType !== 'SINCOSDEFWALK' && questions[currentQuestionIndex].questionType !== 'LEGFINDWALK' && questions[currentQuestionIndex].questionType !== 'TRIGSCWALK' && questions[currentQuestionIndex].questionType !== 'TRIGTGWALK' && questions[currentQuestionIndex].questionType !== 'TRIGCIRCWALK') {
  ...
  const handleFinishLesson = useCallback(() => {
    ...
    // router.refresh() сбрасывает ВЕСЬ клиентский Router Cache Next.js (не
    // только текущий сегмент) — без этого, если пользователь недавно (в
    // пределах staleTime) уже открывал /learn или /trainer в этой сессии,
    // клиентская soft-навигация туда после прохождения урока могла показать
    // СТАРЫЙ закэшированный RSC-снимок карточки "Квест дня" (пункт "пройди
    // урок тренажёра" ещё не вычеркнут), хотя в БД состояние уже обновлено —
    // баг, пойманный пользователем именно на переходе /trainer → /learn.
    router.refresh()
