...
export type UnitCircleData = {
    ...
    guideValue?: number;
    // Только для 'label' — LaTeX-запись guideValue (например
    // "\\dfrac{\\sqrt{2}}{2}") для засечки-подписи прямо на оси, рядом с
...
const LessonIdPage = async ({ params, searchParams }: Props) => {
    ...
    // МЕНЬШЕ count, если в пуле не хватает элементов (не крашится, не
    // требует ровно N), а SWIPE/SCROLL/CONNECT и так уже откатываются на
    // ASSIST, если после этого элементов меньше нужного — так что "мало,
    // но честно" не ломает рендер. Расширяем до родового (кросс-род)
    // пула ТОЛЬКО если своего рода нет вовсе (0) — раньше расширяли уже
    // при простой НЕХВАТКЕ (kindPool.length < count), из-за чего единица
    // измерения могла попасть в варианты ответа вопроса, ожидающего
    // название величины, просто потому что "своих" названий было мало
    // (например 3 при потребности в 5), а не потому что их не было
    // вовсе — пользователь поймал именно этот случай живьём ("Дж" среди
    // вариантов на "Что измеряется в кг⋅м/с?", хотя это другая единица,
    // не даже сама себе). Кросс-род — только настоящий "не осталось
    // вообще ничего своего рода" fallback (см. баг "3 подряд" в уроке 39
    // выше, где оба "В чём измеряется" вопроса делят один ответ).
    ...
    questions = lessonChallenges.map((t_challenge, index): QuestionType | undefined => {
        ...
        if (t_challenge.type === 'FARADAYWALK') {
            // Тот же самодостаточный принцип, что у LOGCOMBOWALK/
            // LOGDIVWALK — разбор "перевёртыша" логарифма (log_a(b) =
            // 1/log_b(a)) на фиксированном примере 8/2, тренировочные
            // задания генерируются на лету внутри type-logflipwalk.tsx.
            return {
                questionType: 'FARADAYWALK' as const,
                question: t_challenge.question,
                imageSrc: t_challenge.imageSrc,
                options: [],
                numRans: t_challenge.numRans,
        ...
        if (t_challenge.type === 'SINCOSDEFWALK') {
            ...
            // треугольниках (заполнение дроби кликами по числам). См.
            // type-sincosdefwalk.tsx.
            return {
                questionType: 'SINCOSDEFWALK' as const,
                question: t_challenge.question,
                imageSrc: t_challenge.imageSrc,
                options: [],
                numRans: t_challenge.numRans,
