...
// Бегунок, риски на треке и подписи вариантов сидят на ОДНИХ и тех же
// трёх точках (TICK_POSITIONS) — раньше края были всего в THUMB_RADIUS_PX
// (12px) от края трека, но подпись-кнопка (w-[92px], center на этой точке
// через -translate-x-1/2) при таком отступе наполовину вылезала за
// границу экрана (46px подписи - 12px отступа = 34px переполнения) — на
// проде это давало горизонтальный скроллбар на крайних вариантах SCROLL.
// Отступ увеличен до половины ширины подписи (92px/2=46px) — с запасом,
// чтобы подпись целиком помещалась в контейнер при любой ширине экрана.
const LABEL_WIDTH_PX = 92
const EDGE_INSET_PX = LABEL_WIDTH_PX / 2
const TICK_POSITIONS = [`${EDGE_INSET_PX}px`, '50%', `calc(100% - ${EDGE_INSET_PX}px)`]
