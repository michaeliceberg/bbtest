...
// Общая логика привязки — используется и командой /bind КОД, и диплинком
// (t.me/bot?start=bind_КОД), который в один тап открывает бота и сразу
// присылает /start bind_КОД без необходимости печатать что-либо руками.
async function performBind(chatId: string, firstName: string, rawCode: string | undefined, keyboard: TelegramReplyKeyboard) {
...
export async function POST(req: Request) {
    try {
        ...
        // Команда /start (в т.ч. диплинк /start bind_КОД — открыл бота и сразу привязался,
        // без ручного набора команды; см. getBindLink в utils/telegram.ts)
        if (rawText.startsWith('/start')) {
            ...
            await sendMessageToTelegram(
                ...
                `1. Откройте в приложении "Настройки" → "Родителям"\n` +
                `2. Нажмите там кнопку "Открыть в Telegram и привязать" — всё произойдёт само\n\n` +
        ...
        if (rawText.startsWith('/bind')) {
            ...
            await performBind(chatId, firstName, code, keyboard);
            return NextResponse.json({ ok: true });
        }
