...
// Слова-функции и другие зарезервированные латинские слова, которые в
// формулах пишутся обычным текстом без backslash (не LaTeX-командой) —
// их буквы НЕ считаются кандидатами-переменными, даже если формально это
// "слитная последовательность латинских букв" (как m/g/h в mgh). Пример:
// "sin"/"cos" в "qvB sin \alpha". Сюда же — не-функции, а именованные
// константы/пометки, которых в формуле физически не существует как
// отдельных переменных: "const" в "Eмех = ... = const" (закон сохранения
// энергии) раньше не считался словом — регэксп резал его на 5 "букв"
// c/o/n/s/t и предлагал пропустить любую из них ("co?st"), хотя это
// одно нерасчленимое слово "константа", а не произведение переменных.
// Найден живой пример в теме "ЗСИ/ЗСЭ" (t_challenge 2103101011 и др.).
...
const findLetterOccurrences = (latex: string): LetterOccurrence[] => {
	...
	while ((match = re.exec(latex))) {
		...
		// Изолированная буква сразу после "\pi" (только пробел между ними,
		// например "2 \pi k" или "\pi n") — это индекс периода в общем
		// решении тригонометрического уравнения, не переменная: он один и
		// тот же в ЛЮБОМ варианте ответа задачи, поэтому пропуск именно
		// этой буквы ничего не проверяет. В формулах темы "Тригонометрия"
		// (см. scripts/seedTM10.ts) это вообще ЕДИНСТВЕННАЯ буква во всей
		// формуле (цифры вроде "3" в "\pi/3" кандидатами не считаются
		// вообще) — из-за этого INSERT раньше ВСЕГДА предлагал заполнить
		// именно её. Проверка по контексту (что стоит прямо перед буквой),
		// а не по конкретному имени буквы — "k"/"n" в других формулах могут
		// быть настоящей переменной (например k — жёсткость пружины в
		// F=kΔx), там их резонно пропускать. Без этой буквы формула
		// остаётся без единого кандидата → pickInsertBlank откатится на
		// ASSIST (штатное поведение), а не покажет неинтересный пропуск.
		if (run.length === 1 && /\\pi\s*$/.test(latex.slice(0, match.index))) continue

		// Изолированная буква — единственный аргумент функции сразу в
		// скобках после известного имени функции ("cos(x)", "sin(x)",
		// "tg(x)") — тот же класс проблемы, что и \pi-индекс периода выше:
		// в теме "Формулы приведения" ответ — ВСЕГДА "±cos(x)"/"±sin(x)" с
		// ТЕМ ЖЕ аргументом x, что и в вопросе (меняется только имя функции
		// и знак, никогда сам аргумент) — пропуск/порча "x" в "cos(x)"
		// ничего не проверяет (ученик не может ошибиться в том, что здесь
		// стоит "x" — это переписано из условия, не вычислено). Найдено
		// сканом всего корпуса (12 из 48 задач темы "Тригонометрия",
		// подтема "Формулы приведения") — все имеют вид "±cos(x)"/"±sin(x)".
		if (run.length === 1 && latex[match.index - 1] === '(' && latex[match.index + 1] === ')') {
			const before = latex.slice(0, match.index - 1)
			const fnNameMatch = before.match(/([A-Za-z]+)\s*$/)
			if (fnNameMatch && FUNCTION_WORDS.has(fnNameMatch[1].toLowerCase())) continue
		}
